...
#        return page_numbers
    ...
    def get_company_research(self, date): #this date parameter is the latest date in the database
...
#        page_numbers = 2
        ...
        while count <= page_numbers:
            ...
            if len(df_tot[df_tot['公告日期']<date]):  # if date is smaller than the latest date in database, no need to turn pages
                pass
                df_tot = df_tot[df_tot['公告日期']>=date]
...
if __name__ == '__main__':
    ...
    result = self.get_company_research()
    print ('Sum')
    print (result)
